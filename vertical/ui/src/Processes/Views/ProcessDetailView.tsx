...
export default function ProcessDetailView() {
  ...
  const enterEditMode = () => {
    ...
    form.reset({
      ...
      hoursPerDay: d?.hoursPerDay ?? (d?.timePerActivity ? (d.timePerActivity / 60) * (d.activitiesPerDay || 1) : 0),
      daysPerWeek: d?.daysPerWeek ?? 5,
      weeksPerYear: d?.weeksPerYear ?? (d?.workingDaysPerYear ? Math.round(d.workingDaysPerYear / 5) : 44),
